...
class Stats:
    ...
    # Allows comparison based on the index of int i and j in the real list, 
    # so based on their positions in the real
    def probe_dict(self, real: dict, i: int, j: int):
        index_i = real.get(i)
        index_j = real.get(j)
        return index_i < index_j
    ...
    def merge_sort_dict(self, real: dict, temp: list[int]):
        if len(temp) <= 1:
            return 0
        ...
        left_invs = self.merge_sort_dict(real, left)
        right_invs = self.merge_sort_dict(real, right)
        between_invs = 0
        i = 0
        j = 0 
        k = 0
        while i < left_size and j < right_size:
            ...
            if self.probe_dict(real, left[i], right[j]):
                temp[k] = left[i]
                k += 1
                i += 1
            else:
    ...
    # Adds the current distance between the real order and our approximation
    def add_curr_distance_dict(self, real: list[int], approx: list[int]):
        temp = approx.copy()
        real_dict = dict(zip(real, range(0, len(real))))

        distance = self.merge_sort_dict(real_dict, temp)
        self.distances.append(distance)